...
export async function buildPersonJsonLd(locale: string) {
  ...
  return {
    ...
    name: profile.name,
    /*
     * The other approved spelling of the same name, not a nickname and not a
     * translation. On /ar `name` is 'سناء رقيق' and this carries 'Sanae Rakik';
     * on the Latin locales they are equal and the property is omitted rather than
     * emitted as a duplicate. Both strings come from data/identity.json, so this
     * still asserts only spellings intake section 6 approved.
     */
    ...(profile.nameLatin && profile.nameLatin !== profile.name
      ? { alternateName: profile.nameLatin }
      : {}),
